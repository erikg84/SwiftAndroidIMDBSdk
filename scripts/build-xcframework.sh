...
#     version      default: VERSION_NAME from gradle.properties
...
# ── Swap to iOS-only Package.swift (strip swift-java which doesn't support iOS) ──
cp "$ROOT/Package.swift" "$ROOT/Package.swift.android-backup"
cat > "$ROOT/Package.swift" <<'IOSMANIFEST'
// swift-tools-version: 6.2
// Temporary iOS-only manifest generated by build-xcframework.sh
...
IOSMANIFEST
log "Swapped to iOS-only Package.swift (stripped swift-java)"

restore_package_swift() {
  if [[ -f "$ROOT/Package.swift.android-backup" ]]; then
    mv "$ROOT/Package.swift.android-backup" "$ROOT/Package.swift"
    log "Restored unified Package.swift"
  fi
}
